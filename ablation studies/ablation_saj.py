...
class SAJBlock(nn.Module):
  def __init__(self,d_model:int,d_inner:int,first:FirstSection,self_attention_block:MultiHeadAttentionBlock,second:SecondSection,insideprojectionlayer:InsideProjectionLayer):
    super().__init__()
    self.first = first
...
class SAJLoop(nn.Module):
  ...
  def forward(self,x,low_triang_mask,device):
    if low_triang_mask:
...
class SAJ(nn.Module):
  ...
  def firstrep(self,x,mask,low_tri_mask,device):
    x = torch.concat([x,mask],dim=-1)
    x = self.input_embedding(x)
    x = self.pos_encoding(x)
    x = self.sajloop(x,low_tri_mask,device)
    return self.firstrepresentation(x)

  def tilde_X1(self,x,mask,low_tri_mask,relu,device):
...
def build_SAJ(seq_len:int=24,features:int=8,d_model:int=64,d_v:int=64,d_inner:int=256,d_conv:int=16,h:int=8,N:int=6,dropout:float=0,conv_dropout:float = 0.1, attn_dropout:float = 0.)->SAJ:
  features = 2*features
  ...
  for _ in range(N):
    ...
    saj_block = SAJBlock(d_model,d_inner,first,self_attention_block,second,insideprojectionlayer)
